...
class MockTab:
    ...
    def __init__(self):
        ...
        self.url_input = MockLineEdit("https://httpbin.org/get")
        self.body_edit = MockTextEdit("")
        self.auth_type = MockComboBox("Bearer Token")
        self.auth_input = MockLineEdit("testtoken")
...
def test_send_request_with_auth():
    tab = MockTab()
    controller = RequestTabController(tab)
    controller.send_request()
    assert tab.response_view.toPlainText() != ""
